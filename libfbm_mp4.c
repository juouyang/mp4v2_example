...
static inline int _write_h264(MP4TrackId* m_videoId, int* sps_wt, int* pps_wt, MP4FileHandle hMp4File, int w, int h, const unsigned char* pData, int size);

int muxer_mp4(const char* in_filename_v, const char* in_filename_a, const char* out_filename) {
    return _muxer_mp4(in_filename_v
            , 1920
            , 1080
            , in_filename_a
            , 48000
...
inline int _muxer_mp4(const char* h264, int width, int height, const char* aac, int sr, const char* mp4) {
    int ret = 0;
    FILE* m_fp_h264 = NULL;
    FILE* m_fp_AAC = NULL;
    ...
    do {
        ...
        while (1) {
            ...
            if (last_update - video_tick_now > video_tick_gap - tick_exp) {
                ...
                video_tick_now = _get_tick_count();
            }

            tick_exp_new = _get_tick_count();
...
static inline int _read_aac(FILE* m_fp_AAC, unsigned char* buf, int buf_size) {
    ...
    int true_size = fread(aac_header, 1, 7, m_fp_AAC);
    if (true_size <= 0) {
        return -2;
...
static inline int _write_h264(MP4TrackId* m_videoId, int* sps_wt, int* pps_wt, MP4FileHandle hMp4File, int w, int h, const unsigned char* pData, int size) {
    ...
    while ((len = _read_one_nalu_from_buf(pData, size, pos, &nalu)) != 0) {
